...
def test_instance_invoke():
    with deploy_app("demo_app.py") as (app, version, instance):
        ...
        url = instance.full_url_for("greet", appHandle=app.handle, useSubdomain=useSubdomain)
        ...
        url = instance.full_url_for("greet?name=Ted", appHandle=app.handle, useSubdomain=useSubdomain)
        resp = requests.get(url, headers=dict(authorization="Bearer {}".format(client.config.apiKey)))
