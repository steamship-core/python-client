...
def create_index(client: Client, pluginInstance: str):
    ...
    index = steamship.create_index(pluginInstance=pluginInstance, upsert=True).data
    assert index is not None

    # Duplicate creation should fail with upsert=False
    task = steamship.create_index(
        handle=index.handle, pluginInstance=pluginInstance, upsert=False
    )
    assert task.error is not None
    assert task.data is None
...
def test_delete_index():
    ...
    index = steamship.create_index(
        pluginInstance=pluginInstance.handle, upsert=True
    ).data
    assert index.id is not None

    task = steamship.create_index(
        handle=index.handle, pluginInstance=pluginInstance.handle, upsert=True
    )
    ...
    task = steamship.create_index(pluginInstance=pluginInstance.handle, upsert=True)
    assert task.error is None
...
def test_index_usage():
    ...
    with _random_index(steamship, pluginInstance.handle) as index:
        ...
        insert_results2 = index.insert(
            A2, externalId=A2id, externalType=A2type, metadata=A2metadata
        )
