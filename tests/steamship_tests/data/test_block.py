...
def test_query(client: Steamship):
    ...
    blocks = Block.query(client=client, tag_filter_query='blocktag and kind "BlockTag"').data.blocks
    assert len(blocks) == 1
    assert blocks[0].id == a.blocks[0].id

    blocks = Block.query(client=client, tag_filter_query='blocktag and kind "Test"').data.blocks
    assert len(blocks) == 1
